/**
 * WF02 R10 Phase A/B/C — hero neighborhood evidence capture.
 *
...
const OUT = '/opt/cursor/artifacts/wf02_r10_neighborhood';
...
const R9_BEFORE = 'Docs/milestones/WF02/01_r9_overview_dawn.png';
const REPO = 'akulasaivineeth/God-Mode-sim';
const RELEASE_TAG_PREFIX = 'review-evidence-wf02-r10';

const SHOTS = [
  { name: '01_r10_overview_dawn', cam: 'overview', simMinute: 0, waitMs: 2800, diagnostics: true },
  { name: '01b_r10_overview_noon', cam: 'overview', simMinute: 360, waitMs: 2800, diagnostics: true },
  { name: '02_r10_angled', cam: 'angled', simMinute: 360, waitMs: 2600, diagnostics: true },
  { name: '03_r10_street_portal', cam: 'street', simMinute: 360, waitMs: 2600, diagnostics: true },
  { name: '04_r10_civic', cam: 'civic', simMinute: 360, waitMs: 2400 },
  { name: '05_r10_residential', cam: 'residential', simMinute: 360, waitMs: 2400 },
  { name: '06_r10_commercial', cam: 'commercial', simMinute: 360, waitMs: 2400 },
  { name: '07_r10_store_workshop', cam: 'store-workshop', simMinute: 360, waitMs: 2400 },
  { name: '08_r10_home_street', cam: 'home-street', simMinute: 360, waitMs: 2600 },
  { name: '09_r10_store_street', cam: 'store-street', simMinute: 360, waitMs: 2600 },
  { name: '10_r10_workshop_street', cam: 'workshop-street', simMinute: 360, waitMs: 2600 },
  { name: '11_r10_future_lot', preset: { position: [0, 12, 32], target: [0, 1, 26] }, simMinute: 360, waitMs: 2000 },
  { name: '12_r10_night', cam: 'overview', simMinute: 870, waitMs: 2600 },
  { name: '13_r10_street_scale_proof', cam: 'street', simMinute: 420, waitMs: 3200, diagnostics: true },
  { name: '14_r10_home_door_scale', cam: 'home-street', simMinute: 420, waitMs: 3200 },
  { name: '15_r10_store_door_scale', cam: 'store-street', simMinute: 420, waitMs: 3200 },
  { name: '16_r10_workshop_door_scale', cam: 'workshop-street', simMinute: 420, waitMs: 3200 },
];
...
async function main() {
  ...
  const comparePath = path.join(OUT, 'compare_r9_r10_northstar.png');
  await buildCompareStrip(
    [
      { label: 'R9 BEFORE (blocked)', file: existsSync(R9_BEFORE) ? R9_BEFORE : path.join(OUT, '01_r10_overview_dawn.png') },
      { label: 'R10 AFTER', file: path.join(OUT, '01_r10_overview_dawn.png') },
      { label: 'NORTH STAR', file: NORTH_STAR },
    ],
    comparePath,
  );

  const manifestPath = path.join(OUT, 'r10_neighborhood_manifest.json');
  await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
  await copyFile(manifestPath, path.join(DOCS_OUT, 'r10_neighborhood_manifest.json'));
  await copyFile(comparePath, path.join(DOCS_OUT, 'compare_r9_r10_northstar.png'));
