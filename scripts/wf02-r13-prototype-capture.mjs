...
async function main() {
  ...
  const comparePath = path.join(OUT, 'compare_r11_r12_r13_northstar.png');
  await buildCompareStrip(
    [
      { label: 'R11 (modular grid)', file: R11_BEFORE },
      { label: 'R12 (authored depth)', file: R12_BEFORE },
      { label: 'R13 (shell prototype)', file: path.join(OUT, '01_r13_overview_dawn.png') },
  ...
  const manifestPath = path.join(OUT, 'r13_prototype_manifest.json');
  await writeFile(manifestPath, JSON.stringify(manifest, null, 2));
  await copyFile(manifestPath, path.join(DOCS_OUT, 'r13_prototype_manifest.json'));
  await copyFile(provenancePathSafe(), path.join(DOCS_OUT, 'r13_shell_provenance.json')).catch(() => {});
  await copyFile(comparePath, path.join(DOCS_OUT, 'compare_r11_r12_r13_northstar.png'));
