/**
 * M02 closure evidence (R14) — converged R12 harness + R13 gameplay scale + fail-closed proof.
 *
...
async function captureIdleAt1x(page, saveShot) {
  ...
  const { meta, lockedActivity } = await acquireAndSettleAt1x(page, {
    preset: 'home-street',
    advanceSpeed: 0,
    activityPattern: /^(Idle|Relaxing)$/i,
    expectedPose: 'idle',
    expectedClip: /^idle$/i,
...
async function publishRelease(scaleInfo, hashResults) {
  ...
  const notes = [
    'M02 R14 — converged R12 harness + R13 gameplay scale + fail-closed closure proof',
    '',
  ...
  execSync(
    `gh release create ${RELEASE_TAG} --repo ${repo} --title "M02 R14 closure evidence" --notes "${notes.replace(/"/g, '\\"')}" ${fileArgs}`,
    { stdio: 'inherit' },
  );
